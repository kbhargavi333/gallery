...
const UserList = () => {
  ...
  const handleFav = ( imageUrl, text) => {
    return async () => {
      try {
        const response = await axios.post("http://localhost:4000/favstore", {
          favimage: imageUrl,
          favtext: text
        });
        console.log(text);
  ...
  const handledel = (imageurl,text,id) => {
    return async () => {
      try {
        const response1 = await axios.delete(`http://localhost:4000/users/${id}`);
        const response = await axios.post('http://localhost:4000/delstore',{
          delimage:imageurl,
          deltext:text
        });
